...
/* 
         ...
         Problema número 26:
           ...
           además de mostrar un listado con todos los alumnos y sus medias,
			  ...
			  diez.Implementar la solucion con una matriz que contenga las notas
			  del alumno en vez de usar un vector.
*/
/*********************************************************************/
...
struct RegAlumno {
	...
	double media = 0;
	double tablanotas[MAX_ALUMNOS][MAX_NOTAS];
	...
	public:
		...
		void Aniade(string nombre){
			...
			for(int i = 0 ; i < num_notas; i++){
				...
				cin >> nota;
				alumno[num_alumno].tablanotas[num_alumno][i] = nota;
				
				peso = pesos[i];
				media = media + ( alumno[num_alumno].tablanotas[num_alumno][i]
				 * ( peso*1.0 /100.0 ) );
